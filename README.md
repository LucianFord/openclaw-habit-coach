...
23:00  cron → agent calls habit_progress + habit_report → delivers review
```

The `gateway_start` hook automatically registers these two cron jobs on startup.
Times are configured via `checkinTime` and `reviewTime`.
