...
const parameters = Type.Object({
    ...
    period: Type.Optional(Type.Union([Type.Literal('weekly'), Type.Literal('monthly')], {
...
function analyzePatterns(entries) {
    ...
    for (const entry of entries) {
        for (const task of entry.tasks) {
            if (!byCat[task.category])
                byCat[task.category] = { done: 0, total: 0 };
            byCat[task.category].total++;
            if (task.completed)
                byCat[task.category].done++;
        }
    }
...
function generateSuggestions(entries) {
    const suggestions = [];
    if (entries.length === 0) {
...
export const habitReportTool = {
    name: 'habit_report',
    description: 'Generate a detailed weekly or monthly report with completion rates, streaks, best/worst days, category breakdown, pattern analysis, and actionable suggestions.',
    parameters,
    async execute(_id, params) {
        try {
            ...
            return {
                content: [{ type: 'text', text: `❌ Error generating report: ${message}` }],
                isError: true,
            };
        }
    },
