...
function generateIssueId(category) {
    const slug = category.replace(/[^a-zA-Z\u4e00-\u9fff]/g, '').slice(0, 6);
    return `${slug}-${Date.now()}`;
}
export const habitSetGoalTool = {
    name: 'habit_set_goal',
    description: "Set a new habit goal or issue for the user to work on. This adds it to the active issues list, which will generate daily tasks going forward. Use when the user wants to start tracking a new habit or improve in a specific area.",
    parameters,
    async execute(_id, params) {
        try {
            ...
            const existing = state.issues.find(i => i.status === 'active' &&
            ...
            const severityLabel = newIssue.severity === 'high'
                ? '🔴 High priority'
            ...
            return {
                content: [{ type: 'text', text: `❌ Error setting goal: ${message}` }],
                isError: true,
            };
        }
    },
