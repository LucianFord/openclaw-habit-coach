import type { HabitState, TaskEntry, DailyLogEntry } from './types.js'
...
function getRecentCategoryCompletionRate(
  dailyLog: DailyLogEntry[],
  category: string,
  days = 3,
): number {
  const recent = dailyLog.slice(-days)
  if (recent.length === 0) return -1 // no data → signal first-run
  ...
  if (total === 0) return -1 // no category data → signal first-run
  return completed / total
}

function determineDifficulty(completionRate: number): Difficulty {
  if (completionRate < 0) return 'easy'      // No history → start easy
  if (completionRate < 0.34) return 'easy'   // Struggling → make it easier
  if (completionRate > 0.80) return 'hard'   // Succeeding → push harder
  return 'medium'
}
...
export function generateTasks(state: HabitState): TaskEntry[] {
  ...
  const highStreaks = state.streaks.filter(
    s => s.status === 'active' && s.currentStreak >= 5,
  )
