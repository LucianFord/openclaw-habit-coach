...
export const getTasksTool: ToolDefinition<typeof parameters> = {
  name: 'habit_tasks',
  description:
    "Get today's dynamically generated tasks for the user. Tasks adapt to recent performance — easier if struggling, harder if succeeding. Call this at the start of each day or when the user asks what they need to do.",
  parameters,
  async execute(_id: string, params: Params) {
    try {
      ...
      return {
        content: [{ type: 'text' as const, text: `❌ Error loading tasks: ${message}` }],
        isError: true,
      }
    }
  },
