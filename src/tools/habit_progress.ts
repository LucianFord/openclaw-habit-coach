...
function formatBar(rate: number, width = 20): string {
  const filled = Math.round(rate * width)
  return '[' + '█'.repeat(filled) + '░'.repeat(width - filled) + ']'
}

export const habitProgressTool: ToolDefinition<typeof parameters> = {
  name: 'habit_progress',
  description:
    "View the user's current progress: active habits, streaks, recent completion rates, and issue statuses. Use this to give the user a quick summary of where they stand.",
  parameters,
  async execute(_id: string, params: Params) {
    try {
      ...
      return {
        content: [{ type: 'text' as const, text: `❌ Error loading progress: ${message}` }],
        isError: true,
      }
    }
  },
